...
/**
 * Layout a tree as ReactFlow nodes / edges using d3-hierarchy.
 *
 * Pure: no React, no DOM, no store reads. Deterministic — same input ⇒ same
 * positions — so memoizing on the input root is enough.
 */
...
export function layoutTree(root: Node): { nodes: FlowNode[]; edges: Edge[] } {
  ...
  const flowNodes: FlowNode[] = hier.descendants().map((d) => ({
    id: d.data.id,
    type: "treeNode",
    // d3-hierarchy uses (x, y) along the cross axis / depth axis; ReactFlow's
    // (x, y) is screen-space, so we swap to render the tree horizontally.
